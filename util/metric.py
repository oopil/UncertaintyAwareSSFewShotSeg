...
class Metric(object):
    ...
    def record(self, pred, target, labels=None, n_run=None):
        ...
        if labels is None:
            ...
            labels = [0,] + labels #list(labels.numpy())
        ...
        for j, label in enumerate(labels):
            ...
            target_map = (target == j).float()
            valid_map = (target != 255).float()

            if target_map.sum() > 0:
                pred_valid = (pred == j).float() * valid_map
                inter = (target_map * pred_valid).sum().detach().cpu().numpy()
                fp = torch.clamp(pred_valid - target_map, min=0).sum().detach().cpu().numpy()
                fn = torch.clamp(target_map - pred_valid, min=0).sum().detach().cpu().numpy()

                tp_arr[label] = float(inter)
                fp_arr[label] = float(fp)
                fn_arr[label] = float(fn)

        self.tp_lst[n_run].append(tp_arr)
        self.fp_lst[n_run].append(fp_arr)
        self.fn_lst[n_run].append(fn_arr)

        mean_iou = tp_arr / (tp_arr + fp_arr + fn_arr)
        # pdb.set_trace()
        curr_iou = np.nansum(mean_iou[labels][1:])
        return curr_iou
