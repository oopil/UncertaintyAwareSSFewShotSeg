...
def main(_run, _config, _log):
    ...
    if _config['model']['part']:
        model = FewshotSegPartResnet(pretrained_path=_config['path']['init_path'], cfg=_config)
    ...
    if _config['fix']:
        ...
        # optimizer = torch.optim.RMSprop(model.parameters(), lr=_config['optim']['lr'])
    ...
    for i_iter, sample_batched in enumerate(trainloader):
        ...
        # check = F.interpolate(support_fg_mask[0][0].unsqueeze(0), size=417//8, mode='nearest').sum()
        # print(check)
        # pdb.set_trace()
