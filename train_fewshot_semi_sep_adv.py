...
from models.FewShotSegPartResnetDistSemiSepAdv import FewShotSegPartDist, FewShotRefine

# from models.FewShotSegResnet import FewShotSeg as FewshotSegResnet
# from models.FewShotSegPartResnet import FewShotSegPartDist as FewshotSegPartResnet
# from models.FewShotSegPartResnetDistRefine import FewShotSegPartDist as FewshotSegPartResnetDist
...
def main(_run, _config, _log):
    ...
    dist_model = FewShotSegPartDist(pretrained_path=_config['path']['init_path'], cfg=_config)
    _log.info('Model: FewShotSegPartDist')
    dist_model = nn.DataParallel(dist_model.cuda(), device_ids=[_config['gpu_id'],])
    ## for 4 fold training
    ckpt = f'{_config["dist_ckpt_dir"]}/{_config["task"]["n_shots"]}s_f{_config["label_sets"]}.pth'
    print(f'Load {ckpt}')
    dist_model.load_state_dict(torch.load(ckpt, map_location='cpu'))
    dist_model.eval()

    # pdb.set_trace()
    refine_model = FewShotRefine(cfg=_config)
    _log.info('Model: FewShotRefine')
    refine_model = nn.DataParallel(refine_model.cuda(), device_ids=[_config['gpu_id'],])
    ...
    if _config['is_refine']:
        _log.info('###### Set optim ######')
        print('Optimizer: Not fix')
        optim = torch.optim.SGD(refine_model.parameters(), **_config['optim'])
    ...
    for i_iter, batch in enumerate(trainloader):
        ...
        unlabel_images = [[unlabel_images.cuda() for unlabel_images in way] for way in batch['unlabel_images']]
        ...
        with torch.no_grad():
            pred = dist_model(support_images, support_fg_mask, support_bg_mask, query_images, unlabel_images)

        query_pred, mask = refine_model(pred, dist_model.module.decoder)
            ...
            with torch.no_grad():
                for run in range(1):
                    ...
                    if meanIoU > highest_iou:
                        ...
                        highest_iou = meanIoU
                        highest_iter = i_iter
                        torch.save(refine_model.state_dict(), os.path.join(f'{_config["ckpt_dir"]}/best.pth'))
                    else:
                        print(f'The highest iou is in iter: {highest_iter} : {highest_iou}')
            torch.save(refine_model.state_dict(), os.path.join(f'{_config["ckpt_dir"]}/{i_iter + 1}.pth'))
        refine_model.train()
    ...
    transforms = [Resize(size=_config['input_size'])]
    transforms = Compose(transforms)
    ckpt = os.path.join(f'{_config["ckpt_dir"]}/6000.pth')
    print(f'{_config["ckpt_dir"]}/6000.pth')

    if _config['is_refine']:
        if _config['pre_refine']:
            ckpt = f'{_config["refine_ckpt_dir"]}/{_config["task"]["n_shots"]}s_f{_config["label_sets"]}.pth'
        refine_model.load_state_dict(torch.load(ckpt, map_location='cpu'))
        
    refine_model.eval()
    refine_model.n_unlabels = _config['test_unlabels']
        ...
        for run in range(5):
            ...
            for batch in tqdm.tqdm(testloader):
                if _config['dataset'] == 'COCO':
                    ...
                    label_ids = list(batch['class_ids'])
                support_images = [[shot.cuda() for shot in way] for way in batch['support_images']]
                suffix = 'mask'
                support_fg_mask = [[shot[f'fg_{suffix}'].float().cuda() for shot in way] for way in batch['support_mask']]
                support_bg_mask = [[shot[f'bg_{suffix}'].float().cuda() for shot in way] for way in batch['support_mask']]
