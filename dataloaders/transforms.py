...
class RandomMirror(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.5:
            ...
            # sb_mask = sb_mask.transpose(Image.FLIP_LEFT_RIGHT)
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        # sb_mask = tr_F.resize(sb_mask, self.size, interpolation=Image.NEAREST)
...
class ToTensorNormalize(object):
    ...
    def __call__(self, sample):
        ...
        # sb_mask = torch.Tensor(np.array(sb_mask)).long()
        inst = torch.Tensor(np.array(inst)).long()
        scribble = torch.Tensor(np.array(scribble)).long()
        # segment = torch.Tensor(np.array(sample['segment'])).float()
