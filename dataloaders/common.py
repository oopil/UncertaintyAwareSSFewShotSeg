...
class BaseDataset(Dataset):
    ...
    def subsets(self, sub_ids, sub_args_lst=None):
        ...
        if sub_args_lst is not None:
            subsets = [Subset(dataset=self, indices=index, sub_attrib_args=args, cls_ids=sub_id_cls, ids=self.ids)
                       for index, args, sub_id_cls in zip(indices, sub_args_lst, sub_ids)]
        else:
...
class Subset(Dataset):
    ...
    """
    def __init__(self, dataset, indices, sub_attrib_args=None, cls_ids=None, ids=None):
    ...
    def remove_from_indices(self, remove_list):
        for e in remove_list:
            if e in self.cls_ids:
                idx = self.ids.index(e)
                print(f"{e} is in ids. check indices.")
            else:
                print(f"{e} is not in ids.")
                continue
                
            if idx in self.indices:
                self.indices.remove(idx)
                print(f"{e} is in indices. remove it.")
            else:
                print(f"{e} is not in indices.")

                
    def check_mask_size(self):
        self.cls_id = self.sub_attrib_args['basic']['class_id']
        list_to_remove = []
        for cnt, id in enumerate(self.cls_ids):
            # pdb.set_trace()
            idx = self.ids.index(id)
            sample = self.__getitem__(cnt)
            fg_mask = sample['label'][self.cls_id].unsqueeze(0).unsqueeze(0)
            # fg_mask = F.interpolate(fg_mask, size=(53,53), mode='nearest')
            area = fg_mask.sum()
            if area < 1:
                list_to_remove.append(id)
            print(f'{cnt}/{len(self.cls_ids)}', end='\r')

        print(f"class {self.cls_id} remove list.", list_to_remove, sep='\n')
        # assert False
