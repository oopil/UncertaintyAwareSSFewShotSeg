...
class VOC(BaseDataset):
    ...
    def __getitem__(self, idx):
        # Fetch data
        id_ = self.ids[idx]
        # image = Image.open(osp.join(self._base_dir, 'boruvkasupix_3200', f'{id_}.jpg'))
        ...
        with open(osp.join(self._base_dir, 'superpixel', f'{id_}.pkl'), 'rb') as load_f:
            segments = pickle.load(load_f)
        segments = Image.fromarray(segments)
