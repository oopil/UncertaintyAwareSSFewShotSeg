...
class FewShotRefine(nn.Module):
    def __init__(self, cfg=None):
        ...
        self.config = cfg
        self.hdim = 2048
        self.low_level_ft = 1024#512
        self.iter = cfg['refine_iter']
        self.refiner = RefineNet(in_dim=self.low_level_ft, \
            h_dim=self.low_level_ft, dilations = [6,12,18],\
            is_pcm=cfg["is_pcm"], is_sigma=cfg["is_sigma"]) #[6,12,18], [3,6,9]
        self.out_size = cfg['input_size']

    def forward(self, pred):
        # pdb.set_trace()
        mean, sigma, qft = pred["mean"], pred["sigma"], pred["qft"]
        output_refine = self.refiner(qft, mean, sigma, n_iter=1)
        output_refine = F.interpolate(output_refine, size=self.out_size, mode='bilinear', align_corners=True)
        # output_refine = F.softmax(output_refine,dim=1)
        return output_refine
...
class FewShotSegPartDist(nn.Module):
    ...
    def __init__(self, pretrained_path=None, cfg=None):
        ...
        self.decoder = Decoder(dim=self.hdim, sigma_min=0.3, sigma_scale=1)
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        for epi in range(batch_size):
            ...
            sigma = self.decoder.forward([bg_selected_proto, fg_selected_proto], qry_fts[:, epi])
            sigmas.append(sigma)
