...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None):
        ...
        #     ('backbone', Encoder(in_channels)), ]))
        self.device = torch.device('cuda')
        # self.attention = SetTransformer(dim_input=512, num_outputs=53*53, dim_output=512,
        #     num_inds=32, dim_hidden=128, num_heads=4)
        self.attention = Self_Attn(2048,None)
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        # img_fts, map = self.attention(img_fts)
        ...
        for epi in range(batch_size):
            ...
            # pdb.set_trace()
            qry_ft = qry_fts[:,epi,:]#.view(1,512,53*53)
            qry_ft, map = self.attention(qry_ft)
            # qry_ft = qry_ft.view(1,512,53,53)

            ###### Compute the distance ######
            prototypes = [bg_prototype,] + fg_prototypes
            dist = [self.calLogits(qry_ft, prototype) for prototype in prototypes] #3, 1*53*53
            # dist = [self.calLogits(qry_fts[:, epi], prototype) for prototype in prototypes] #3, 1*53*53
    ...
    def calLogits(self, qft, proto, scaler=1/10):
        """
        :param qft: [N, C, H, W]
        :param proto: [1, C]
        :return:
        """
        logits = qft*(proto.unsqueeze(2).unsqueeze(3))
        logits = logits.sum(dim=1)/proto.norm()
